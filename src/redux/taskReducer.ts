...
export const tasksReducer = (tasks: TasksType = {}, action: TsarType): TasksType => {
  switch (action.type) {
    case "SET-LISTS": {
      let tasksObj = { ...tasks };
      action.lists.map((l) => (tasksObj = { ...tasksObj, [l.id]: [] }));
      return tasksObj;
    }
    case "SET-TASKS": {
    ...
    case "REMOVE-TASK": {
      return {
        ...tasks,
        [action.listId]: tasks[action.listId].filter((t) => t.id !== action.id),
      };
    }
    case "ADD-TASK": {
      // const d = new Date()
      ...
      return {
        ...tasks,
        [action.listId]: [...tasks[action.listId], action.task],
      };
    }
    case "CHANGE-TASK-STATUS": {
      return {
        ...tasks,
        [action.listId]: tasks[action.listId].map((t) => (t.id === action.id ? { ...t, isDone: action.isDone } : t)),
      };
    }
    case "RENAME-TASK": {
      return {
        ...tasks,
        [action.listId]: tasks[action.listId].map((t) => (t.id === action.id ? { ...t, title: action.title } : t)),
      };
    }
    case "ADD-LIST": {
...
export const removeTaskAC = (listId: string, id: string) => ({ type: "REMOVE-TASK", listId, id } as const);
export const addTaskAC = (listId: string, task: TaskType) => ({ type: "ADD-TASK", listId, task } as const);
